(function () {
  ...
  if (!categories.some(function (category) { return category.id === selected; })) selected = "all";
  var query = "";
  ...
  var tabData = [{ id: "all", name: "All" }].concat(categories);
  tabs.innerHTML = tabData.map(function (category) {
    return '<button class="pommy-category-tab" type="button" data-category-filter="' + category.id + '" aria-pressed="' + String(category.id === selected) + '">' + category.name + '</button>';
  }).join("");
