(function () {
  ...
  function renderLocalMenuLanding(category, title, intro) {
    var items = menu.filter(function (item) { return item.category === category && item.available !== false; });
    replaceMain('<main id="main-content">' +
      section('<div class="pommy-page-intro"><div class="pommy-eyebrow">Pommy Burger and Pizza</div><h1>' + escapeHtml(title) + '</h1><p>' + escapeHtml(intro) + '</p><div class="pommy-actions" style="justify-content:center"><a href="/menu/?category=' + escapeHtml(category) + '" class="button-primary w-button">Browse ' + escapeHtml(category === "burger" ? "Burgers" : "Pizza") + '</a><a href="/contact/" class="button-secondary w-button">Location & Contact</a></div></div><div class="pommy-menu-grid">' + items.map(productCard).join("") + '</div>', "") +
      section('<div class="pommy-two-column"><div><h2>Pommy around CMC, Addis Ababa</h2><p>Pommy Burger and Pizza serves dine-in and takeaway customers around CMC. Use the confirmed plus code for directions, call the restaurant, or review the full menu before ordering.</p></div><div class="pommy-actions"><a href="' + escapeHtml(config.directionsUrl) + '" target="_blank" rel="noopener" class="button-primary w-button">Get Directions</a><a href="tel:' + escapeHtml(config.phoneInternational) + '" class="button-secondary w-button">Call Pommy</a><a href="/checkout/" class="button-secondary w-button">Order</a></div></div>', "bg-secondary-1") +
      '</main>');
  }
