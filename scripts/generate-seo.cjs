...
const llms = [
  ...
  `- Phone: ${business.phoneDisplay} (${business.phoneInternational})`,
  `- Food: ${business.cuisines.join(", ")}`,
  `- Service area: ${business.areaServed.join(", ")}`,
  `- Payment: ${business.paymentAccepted}`,
  "- Opening hours: Not confirmed; call Pommy before visiting.",
  "- WhatsApp: No verified public number is configured.",
  "",
  "## Canonical pages",
  "",
  `- Home: ${absolute("/")}`,
  `- Menu and current ETB prices: ${absolute("/menu/")}`,
  `- Burgers around CMC: ${absolute("/burger-around-cmc/")}`,
  `- Pizza around CMC: ${absolute("/pizza-around-cmc/")}`,
  `- Order: ${absolute("/checkout/")}`,
  `- Location and contact: ${absolute("/contact/")}`,
  `- About: ${absolute("/about/")}`,
  `- Blog: ${absolute("/blog/")}`,
  "",
  "The menu contains 101 items across 10 categories. Product prices are displayed in ETB. No reviews, ratings, awards, coordinates, opening hours, email address or social profiles are claimed without owner verification.",
  ""
].join("\n");
